...
def encode_col_features(X,col):
    """Converts categorical values in column 'col' of X to integers in the range
    [0, n_unique_values_in_column - 1], if X is not already of integer type.
    ...
    during encoding, and get treated as unknowns during prediction.
    """
    values = enumerate(np.unique(X.loc[:, col]))
    col_enc = {val: jj for jj, val in values if not (isinstance(val, float) and np.isnan(val))}
    X.loc[:, col] = np.array([col_enc.get(x, -1) for x in X.loc[:, col]])
        ...
        for k in gen:
            ...
            if visualize:
                ...
                fig = add_subplot(fig=fig, row=g, col=k,
                                  ...
                                  tv=tv, text=text)
    if visualize:
        save_at = "{0}/All_num_nc={1}.pdf".format(result_dir, n_cluster)
        makedirs(save_at)
        plt.savefig(save_at)
...
def run(argv):
    ...
    tv = "Lattice_Constant"
    cat_cols=["Atomic_Number_A", "Atomic_Number_B", "atom_A", "atom_B", "group_A", "group_B", "period_A", "period_B", "group_index"]
    num_clusters = 3
    init='random'
    n_init = 50
    input_dir = "input"
    ...
    save_result_dir = result_dir + "/{0}/All_num_nc={1}_init={2}".format(only_name, num_clusters,init)
    makedirs(save_result_dir)
    gidx_file = "{0}/All_num_nc={1}.csv".format(save_result_dir,num_clusters)
    makedirs(gidx_file)

    data_df = pd.read_csv(source_file, index_col=0)
    #Check if the dataframe contains any non-numeric values, if true, then convert categorical columns into numerical ones
    check_data_df= data_df.loc[:,cat_cols]
    if check_data_df.shape[1] != check_data_df.select_dtypes(include=np.number).shape[1]:
        for col in cat_cols:
            if is_string_dtype(check_data_df[col]):
                print(col)
                encode_col_features(data_df, col)
