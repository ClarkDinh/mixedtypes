        ...
        for k in gen:
            ...
            if visualize:
                ...
                fig = add_subplot(fig=fig, row=g, col=k,
                                  ...
                                  tv=tv, text=text)
    if visualize:
        save_at = "{0}/All_cat_nc={1}.pdf".format(result_dir, n_cluster)
        makedirs(save_at)
        plt.savefig(save_at)
...
def run(argv):
    ...
    save_result_dir = result_dir + "/{0}/All_cat_nc={1}_init={2}".format(only_name, num_clusters,init)
    makedirs(save_result_dir)
    gidx_file = "{0}/All_cat_nc={1}.csv".format(save_result_dir,num_clusters)
    makedirs(gidx_file)

    data_df = pd.read_csv(source_file, index_col=0)
    # Get the list of numerical columns
    convert_to_cat_df = data_df.drop(cat_cols, axis=1)
    num_cols = convert_to_cat_df.columns
    print(num_cols)
    convert_to_cat_df = discretize_col_features(convert_to_cat_df)
    data_clustering_df = pd.DataFrame(columns= data_df.columns, index=data_df.index)
    data_clustering_df[num_cols] = convert_to_cat_df
    data_clustering_df.loc[:,cat_cols] = data_df.loc[:,cat_cols]
    X = np.array(data_clustering_df.drop([tv], axis=1))
    print(X)
    # X, enc_map = encode_features(X)
    clusterer = KModes(n_clusters=num_clusters, init=init, n_init=n_init, verbose=1)
