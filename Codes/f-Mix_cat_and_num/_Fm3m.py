        ...
        for k in gen:
            ...
            if visualize:
                ...
                fig = add_subplot(fig=fig, row=g, col=k,
                                  ...
                                  tv=tv, text=text)
    if visualize:
        save_at = "{0}/pro_nc={1}.pdf".format(result_dir, n_cluster)
        makedirs(save_at)
        plt.savefig(save_at)
...
def run(argv):
    ...
    save_result_dir = result_dir + "/{0}/pro_nc={1}_init={2}".format(only_name, num_clusters,init)
    makedirs(save_result_dir)
    gidx_file = "{0}/pro_nc={1}.csv".format(save_result_dir,num_clusters)
    makedirs(gidx_file)

    data_df = pd.read_csv(source_file, index_col=0)
    X = np.array(data_df)
    
    cat_cols_index = [data_df.columns.get_loc(c) for c in cat_cols if c in data_df]
    print(cat_cols_index)
    kproto = KPrototypes(n_clusters=num_clusters, init=init, verbose=verbose)
    clusters = kproto.fit_predict(X, categorical=cat_cols_index)
